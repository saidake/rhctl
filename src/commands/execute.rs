...
pub async fn run(
    ...
    if config.use_sudo {
        let temp_remote_dir = global_server_pool
            .create_remote_temp_dir(server_metadata,"exec", config.use_sudo)
            .await?;
        log_debug!(
        ...
        global_server_pool
            .exec(server_metadata,&format!("rm -rf {}", temp_remote_dir), config.use_sudo)
            .await?;
    } else {

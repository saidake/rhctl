...
pub fn run(session: &SshSession, config: &Config, local_patch: &str, remote_upload: &str, remote_file: &str, remote_backup: &str, recover: bool) -> Result<(), String> {
    ...
    if recover {
        ...
        session.execute(&format!("cp {} {} -f", resolved_backup, resolved_file), config.sudo)?;
    ...
    session.scp_upload(&local_path, &resolved_upload, config.sudo)?;
    ...
    session.execute(&format!("cp {} {} -f", resolved_file, resolved_backup), config.sudo)?;

    // Overwrite
    if !config.silent && !ask_user(&format!("Overwrite {} with {}? (y/n): ", remote_file, remote_upload)) {
        return Ok(());
    }
    session.execute(&format!("cp {} {} -f", resolved_upload, resolved_file), config.sudo)?;
